...
if(ini_get('allow_url_fopen') != false){ 
	if ($df_ddi_version = file_get_contents("http://www.krismods-fusion.pl/updates/news_center_panel.txt")) {
		if (preg_match("/^[0-9a-z\.]+$/", $df_ddi_version)) {
		    $in_version = dbarray(dbquery("SELECT inf_version FROM ".DB_INFUSIONS." WHERE inf_folder = 'news_center_panel'"));	
