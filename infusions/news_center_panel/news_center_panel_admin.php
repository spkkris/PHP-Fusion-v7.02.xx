...
+--------------------------------------------------------+
| Plik: news_center_panel_admin.php
| Autor: krystian1988
| Wersja: 1.00
+--------------------------------------------------------+
...
if (!checkrights("KMFN") || !defined("iAUTH") || $_GET['aid'] != iAUTH) { redirect("../../index.php"); }
...
include INFUSIONS."news_center_panel/version_check.php";
...
$kmfn_ustawienia = dbarray(dbquery("SELECT * FROM ".DB_KMF_DCP));
	...
	$wylacz = "<img src='".INFUSIONS."news_center_panel/img/off.png' alt='".$locale['wylacz']."' class='admin-icons'/>";
...
echo"<div style='text-align: center;' class='admin-message center'>".$locale['admin']."</div>";
		...
		echo "<td  align='left' class='tbl'><input name='ile' type='number' min='1' max='255' style='width: 70px' value='".$kmfn_ustawienia['ile']."' class='textbox'>\n";
		echo "</input>* Max. 255";
		echo "</td></tr><tr>\n";
		echo "<td  class='tbl' width='50%' align='left'>".$locale['023']."</td>\n";
		echo "<td  align='left' class='tbl'><select name='slider' class='textbox'>\n";
		echo "<option value='1'".($kmfn_ustawienia['slider'] == "1" ? " selected='selected'" : "").">".$locale['wlacz']."</option>\n";
		echo "<option value='0'".($kmfn_ustawienia['slider'] == "0" ? " selected='selected'" : "").">".$locale['wylacz']."</option>\n";
		echo "</select>";
		echo ($kmfn_ustawienia['slider'] == 1 ? $wlacz : $wylacz);
		echo "</td></tr><tr>\n";
		echo"</table>\n";
		echo $zapisz;
		echo "</form>\n";
